  ...
  return (
    <div className="dashboard-container">
      <header className="dashboard-header">
        <h1 style={{color:"darkblue"}}>Pharmacist's Page</h1>
      </header>
      <nav className="dashboard-nav">
        <button className={`nav-button ${tab === 'Dashboard' ? 'active' : ''}`} onClick={() => handleTabChange('Dashboard')}>Dashboard</button>
        <button className={`nav-button ${tab === 'Lists' ? 'active' : ''}`} onClick={() => handleTabChange('Lists')}>Lists</button>
        <button className={`nav-button ${tab === 'Users' ? 'active' : ''}`} onClick={() => handleTabChange('Users')}>Users</button>
        <button className={`nav-button ${tab === 'Settings' ? 'active' : ''}`} onClick={() => handleTabChange('Settings')}>Settings</button>
      </nav>
